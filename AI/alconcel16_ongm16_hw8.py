...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        super(AIPlayer,self).__init__(inputPlayerId, "td learning1")
    ...
    def consolidate(self, anticState):
        ...
        newBoard = anticState.board # same
    ...
    #
    #description:
    #saves the current state-utility list to a file called "save"
    #
    def save(self):
    ...
    #
    #description:
    #loads a state-utility list from a file called "save"
    #
    def load(self):
        with open( 'alconcel16_ongm16_utils', 'rb' ) as f:
    ...
    #
    #Description:
    #a simple reward function that rewards the AI for winning, and punishes it for everything else.
    #+1 for winning, -1 for losing, and -0.01 for everything else.
    #
    #parameters:
    #consolidatedState - the current (consolidated) state of the game
    #
    #returns:
    #1 for a win, -1 for a loss, -0.01 for everything else
    def reward(self, consolidatedState):
        if consolidatedState.inventories[0].foodCount >= 11:
            return 1.0 #means we won
        elif consolidatedState.inventories[0].getQueen() is None:
            return -1.0 #means we lost
    ...
    #getFutureMove
    #
    #Description: from the state-utility pairs, picks the state with the best utility
    #
    #Parameters:
    #currentState - the current state of the game
    #
    #Return:
    #a tuple containing the resulting state, and the move to go with it
    def getFutureMove(self,currentState):
        ...
        for m in allMoves:
            s = self.consolidate(self.getFutureState(currentState, m))
            consolidatedStates.append((s,m))

        calculatedStates = []
        for s,m in consolidatedStates:
            if self.utilDic != None:
        ...
        if len(calculatedStates) > 0:
            ...
            next = max(calculatedStates)
            return next,best
        #return a random move if you don't have anything yet
        else:
            return random.choice(consolidatedStates)
    ...
    def getPlacement(self, currentState):
        ...
        if currentState.phase == SETUP_PHASE_1:    #stuff on my side
            ...
            if os.path.isfile("alconcel16_ongm16_utils"): #load previous array
                self.load()
    ...
    #
    #Description:
    #prepares info about the game in advance. For example, where the tunnels and food are.
    #
    #parameters:
    ...
    def getMove(self, currentState):
        ...
        stateMove = self.getFutureMove(currentState)
        newMove = stateMove[1]

        consolidatedState = self.consolidate(currentState)
        nextState = stateMove[0]

        #check if currentState or nextState are already in list
        foundCon = False
        foundNex = False
        for pair in self.utilDic:
            if pair[0] == consolidatedState:
                foundCon = True
            if pair[0] == nextState:
                foundNex = True
        #if you found it, good.
        #otherwise, stick it in there
        if foundCon:
                util = self.utilDic[consolidatedState]
        else:
            # state not found, so add it
            util = self.reward(consolidatedState)
            self.utilDic.append((consolidatedState, util))
            stateCount = 1

        if foundNex:
            x = 0
        else:
            util = self.reward(nextState)
            self.utilDic.append((nextState, util))

        #update utilities using TD-Learning
        count = 0
        index1 = None
        index2 = None
        for pair in self.utilDic:
