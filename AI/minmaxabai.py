...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        super(AIPlayer,self).__init__(inputPlayerId, " aB Minumus Maximus")
        ...
        for x, column in enumerate(self.preProcessMatrix):
            for y, squareProperties in enumerate(column):
                ...
                for food in foodList:
                    stepsToFood = stepsToReach(state, (x, y), food.coords)
                    if stepsToFood < bestDist:
                        bestDist = stepsToFood
                        closestFood = food
                ...
                for constr in constrList:
                    stepsToConstr = stepsToReach(state, (x, y), constr.coords)
                    if stepsToConstr < bestDist:
                        bestDist = stepsToConstr
    ...
    def getPlayerScore(self, hypotheticalState, playerNo, debug=False):
        ...
        for ant in hypotheticalState.inventories[playerNo].ants:
            if ant.type == QUEEN:
                x, y = ant.coords
                if (self.preProcessMatrix[x][y]['foodDist'] == 0 or
                    self.preProcessMatrix[x][y]['constrDist'] == 0
                    ):
